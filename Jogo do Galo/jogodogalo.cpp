...
//Área de definição dos cabeçalhos da função
void menuInicial();
...
void exibeTabuleiro (char tabuleiro[3][3]){
     ...
     //Exibe o tabuleiro com as linhas e colunas e salta a linha ao sair do for
    int linha,coluna;
    cout << "\n";
    for(linha = 0; linha < 3; linha++){
        for(coluna = 0; coluna < 3; coluna++){
...
void jogo(string nomeDoJogadorUm, string nomeDoJogadorDois, int pontuacaoJogadorUm, int pontuacaoJogadorDois){
    ...
    int linha,coluna;                                           //Auxiliares para o tabuleiro
    int linhaJogada,colunaJogada,posicaoJogada;                 //Posição em que o jogador posiciona a sua marca
    ...
    while(rodada < 9 && estadoDeJogo == 1){
        ...
        while(posicionouJogada == false){
            ...
            if(tabuleiro[linhaJogada][colunaJogada] == '-'){
                ...
                //Conseguiu posicionar um lugar
                posicionouJogada = true;
 
                //Verifica de quem é a vez para posicionar o lugar correto
                if(turnoDoJogador == 1){
